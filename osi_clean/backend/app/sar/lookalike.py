...
def assess(seg_conf: float, *, mean_contrast_db: float, elongation: float,
    ...
    if shore_km is not None and not touches_land and shore_km < 5.0:
        pens.append({"rule": "R4b_sheltered_water", "penalty": 0.20,
    ...
    if elongation < 1.6 and area_km2 > 5:
        pens.append({"rule": "R2_blob_shape", "penalty": 0.35,
                     "detail": f"Round, large object (elong {elongation:.1f}, {area_km2:.0f} km²) resembles low-wind cell / rain cell"})

    if mean_contrast_db < 1.5:
        pens.append({"rule": "R3_weak_contrast", "penalty": 0.30,
                     "detail": f"Contrast {mean_contrast_db:.1f} dB below background is weak (biogenic film?)"})

    if touches_land:
        pens.append({"rule": "R4_coastal", "penalty": 0.30,
