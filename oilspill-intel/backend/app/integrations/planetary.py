...
def search(lon: float, lat: float, start: str, end: str, top: int = 10, bbox=None) -> list[dict]:
    ...
    for f in r.json().get("features", []):
        ...
        if p.get("sar:instrument_mode") not in (None, "IW"):
            continue
        # STAC bbox intersect can be loose for tilted footprints: require the true footprint polygon to contain the point
        if not bbox and f.get("geometry"):
            try:
                from shapely.geometry import shape, Point
                if not shape(f["geometry"]).buffer(0.02).contains(Point(lon, lat)):
                    continue
            except Exception:
                pass
