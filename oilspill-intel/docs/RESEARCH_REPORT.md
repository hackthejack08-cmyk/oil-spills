...
**Sentinel-1 without any account (decision change):** Microsoft Planetary Computer serves the full Sentinel-1 GRD
archive as cloud-optimised GeoTIFFs + annotation XML under an anonymous, auto-issued SAS token
(`/api/sas/v1/token/sentinel-1-grd`). Because the measurement TIFFs are tiled with overviews, the calibrator reads
only the AOI window at the requested decimation (≈ 30 MB for a 0.6° box at 40 m) instead of a 1 GB SAFE download.
This is now the **default provider**; CDSE remains as fallback. Verified: item
`S1A_IW_GRDH_1SDV_20250320T010323…`, σ⁰ VV median −19.5 dB over open sea, GCP polynomial fit RMS 0.97 px, 20 s
end-to-end, 520 MB peak RAM. Caveat: the STAC asset named `schema-product-*` points at the RFI annotation on some
items; the calibrator derives the true `annotation/iw-<pol>.xml` path from the measurement URL.

**Land / shoreline handling (added after real-scene test):** Natural Earth 10 m land polygons (public domain,
bundled, 17 MB) are rasterised per scene; a 1 km shoreline buffer is excluded from detection and a
distance-to-shore feature feeds look-alike rules R4/R4b. On the Mumbai scene this removed a 491 km² false
"possible_oil" covering the harbour/creeks and down-weighted all remaining coastal objects (all now ≤ 0.53).
